...
test("renders a verified tclk track record with lifecycle sequence numbers", () => {
  ...
  assert.match(source, /\["receipt", receiptLabel\]/);
  assert.match(source, /SUCCESSFUL · DELIVERY VERIFIED/);
  assert.match(source, /CLAIMED · RECEIPT PRESENT · NO DELIVERY/);
  assert.match(source, /isSuccessfulTrackEntry/);
  assert.match(source, /PAYER RECEIPT PENDING/);
  assert.match(source, /PAYER RECEIPT/);
...
test("recovers a verified payee accept from retained track history", () => {
  ...
  assert.match(source, /candidate\.offer\?\.id && deal\.offer\?\.id && candidate\.offer\.id !== deal\.offer\.id/);
  ...
  assert.match(source, /QUEUE CAPACITY AVAILABLE/);
});
...
test("human-reviewed custom deliveries can be rejected with a signed FAIL review", () => {
  ...
  assert.match(source, /payer-fail-reason/);
  assert.match(source, /manualDeliveryRejectionReason/);
  assert.match(source, /A specific 3-240 character rejection reason is required/);
