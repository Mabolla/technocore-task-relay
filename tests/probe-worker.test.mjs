...
test("closed sonnet-2 lumen continuity performs no network or publish work", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => {
    throw new Error("closed continuity must not fetch");
  };
  try {
    assert.deepEqual(await publishSonnet2LumenContinuityOnce({
      SONNET_2_LUMEN_ENABLED: "true",
      SONNET_2_LUMEN_CLOSED: "true"
    }), { action: "closed" });
  } finally {
    globalThis.fetch = originalFetch;
  }
...
test("Lumen roster consent defaults fail closed", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => { throw new Error("must not fetch"); };
  try {
