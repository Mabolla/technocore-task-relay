...
var CONTRACT_ID2 = /^0x[0-9a-f]{64}$/;
function requireContractId(contract) {
  if (!CONTRACT_ID2.test(contract))
    throw new Error(`tclk: malformed contract id: ${contract}`);
  return contract;
}
...
function makeLivePaperOffer({ from, jobId, now = Date.now() }) {
  ...
  return makeOffer({
    ...
    asset: "PAPER",
    rails: ["paper"],
    claimByMs: now + 48 * 60 * 6e4,
    refundAfterMs: now + 72 * 60 * 6e4,
    expiresMs: now + 24 * 60 * 6e4,
...
async function findValidAccept(raw, offer, now = Date.now()) {
...
async function verifyExactSignedTextRecord(raw, expectedText, from, room) {
  for (const record of records(raw)) {
...
async function listRecentAcceptedPayerDeals(raw, payerDids, now = Date.now(), limitPerPayer = 3) {
  ...
  return [...accepted.values()].sort((left, right) => Number(right.acceptSeq || 0) - Number(left.acceptSeq || 0)).filter((deal) => {
...
async function listSafePaperOffers(raw, myDid, now = Date.now(), minimumFinishMs = 0) {
  ...
  for (const { record, frame } of decoded) {
    ...
    const safeContext = /^\/kv\/[a-z0-9][a-z0-9_-]{0,47}\/[a-z0-9][a-z0-9_-]{0,47}$/.test(frame.job.context) || safePublicHttpsUrl(frame.job.context);
...
async function reviewJobSpec(raw, offer) {
  ...
  if (text.length < 20 || text.length > 2e4) return null;
...
async function listSignedDeliveries(raw, accept, deliveryRoom = dealRoom(accept.contract)) {
...
function makePayerDeliveryReview(offer, accept, from, deliverySeq, verdict, reason) {
...
export {
  ...
  classifyPaperRecord,
  deliveryRoomFromJobText,
  encodeFrame,
  evaluateObjectiveDelivery,
  expectedPaperClaim,
  expectedPaperLock,
  expectedPaperRefund,
  findSignedPayerFailReview,
  findValidAccept,
  foldPayeeDeal,
